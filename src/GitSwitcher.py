...
def init_parser(): 
    ...
    parser = argparse.ArgumentParser(description="A Command Tool to Change Global Git Config.", formatter_class=argparse.RawTextHelpFormatter)
...
def PrintCustomSymbols():
    ...
    if currentUserName != '' and currentUserEmail != '':
        ...
        for section in gitConfig.sections():
            if section.startswith(prefixString):
                if gitConfig.get(section, 'name') == currentUserName and gitConfig.get(section, 'email') == currentUserEmail:
                    currentSymbol = gitConfig.get(section, 'symbol')
                    flag = True
                    break
        if flag:
            print(changeColor('Current User: \n    ', 'yellow') + changeColor(currentSymbol, 'green') + ': ' + currentUserName + ' <' + changeColor(currentUserEmail, 'blue') + '>')
        else:
